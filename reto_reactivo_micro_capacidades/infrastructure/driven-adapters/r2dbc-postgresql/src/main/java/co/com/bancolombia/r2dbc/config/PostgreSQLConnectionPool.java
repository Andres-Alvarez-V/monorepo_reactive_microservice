...
public class PostgreSQLConnectionPool {
	...
	public ConnectionPool getConnectionConfig(PostgresqlConnectionProperties properties) {
        if (properties.host() == null || properties.host().isEmpty()) {
            throw new IllegalArgumentException("Host is required");
        }
        if (properties.port() <= 0) {
            throw new IllegalArgumentException("Port must be a positive number");
        }
        if (properties.database() == null || properties.database().isEmpty()) {
            throw new IllegalArgumentException("Database is required");
        }
        if (properties.schema() == null || properties.schema().isEmpty()) {
